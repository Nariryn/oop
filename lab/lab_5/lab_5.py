...
def drop_from_subject(student, subject):
    ...
    if enrollment is not "Not Found":
...
def assign_grade(student, subject, grade):
    ...
    if enrollment is not None:
        if enrollment.get_grade() is not None:
            return "Error"
        else:
...
def get_teacher_teach(subject_search):
    if subject_search.get_teacher() is None :
        return "Not Found"
    else :
...
def get_student_record(student):
    ...
    if enrollments is not None:
        record = {enrollment.get_subject().get_subject_id(): [enrollment.get_subject().get_subject_name(), enrollment.get_grade()] for enrollment in enrollments}
    else:
        record = {}
...
def get_student_GPS(student):
    ...
    for enrollment in enrollments:
        ...
        if grade:
            credits = subject.get_credit()
            total_credit += credits
            total_grade += credits * grade_to_count(grade)
    if total_grade == 0:
        return 0.00
    return '{:.2f}'.format(total_grade/total_credit)

def list_student_enrolled_in_subject(subject_id):
    subject = search_subject_by_id(subject_id)
    if subject is None:
        return "Subject not found"
    filter_student_list = [enrollment.get_student() for enrollment in enrollment_list if enrollment.get_subject().get_subject_id() == subject_id]
    student_dict = {}
